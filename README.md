...
- Validated for atoms and basis sets involving **s, p, and d character** — including atoms that are not first in the `[Atoms]` block, and cases with polarization d-shells on multiple atom types (e.g. C, N).
- **Not tested for orbital character beyond 3d** — e.g. 4f character on lanthanides/actinides, or 5s/5p/5d character on heavier main-group or transition metal centers. The s/p/d breakdown and offset calculation should extend correctly in principle, but this hasn't been verified against a real system. If you use this on a system with f-shells of chemical interest (rather than just polarization functions), treat the results with appropriate caution and consider spot-checking against an independent source.
- If the atom of interest has f or g shells, the reported s/p/d percentages will not sum to 100% — the remainder reflects f/g character, which isn't broken out as a separate column.
